...
**MCP 0.3.3: карточка поста внутри ИИ-чата.** Сервер предоставляет 32 инструмента. В настоящем ChatGPT в Chrome проверены авторизация, показ карточки, обновление данных и открытие формы планирования. Правки и отправка через интерфейс ChatGPT остаются отдельной проверкой. Полный набор из 798 тестов, включая 58 профильных проверок карточки, прошёл; проверка типов и сборка также успешны. См. [инструкцию карточки](docs/chat-card.md) и [проверенные сценарии](docs/compatibility.md). Запись MCP Registry пока имеет версию `0.2.0-rc.1` и не означает одобрение OpenAI.
