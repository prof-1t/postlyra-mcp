...
**MCP 0.3.0: карточка поста внутри ИИ-чата.** Сервер предоставляет 32 инструмента; 11 сентября 2026 года прошли 776 тестов и публичные проверки SDK и UI-ресурса. Предпросмотр, правки исходника и управление расписанием проверены в локальном MCP Apps host. Отображение в самом ChatGPT и интерфейсах других производителей пока не проверено. См. [инструкцию карточки](docs/chat-card.md) и [ранее проверенные сценарии](docs/compatibility.md). Запись MCP Registry пока имеет версию `0.2.0-rc.1` и не означает одобрение OpenAI.
